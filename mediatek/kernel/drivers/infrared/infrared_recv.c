/*$_FOR_ROCKCHIP_RBOX_$*/
/*$_rbox_$_modify_$_huangzhibao,20120522*/
/*$_rbox_$_modify_$add this file for rk29 remotectl*/
...
//    {0x70, KEY_BACK}, 
    ...
    {0x80, KEY_MENU},     //search
    {0x40, KEY_VOLUMEDOWN},
    {0xc0, KEY_VOLUMEUP},
};


static struct rkxx_remote_key_table remote_key_table_df_ir_test[] = {
    {0x48, KEY_ENTER},
    {0x70, KEY_BACK}, 
    {0xfd, KEY_UP},
    {0xfc, KEY_DOWN},
...
static struct rkxx_remotectl_button remotectl_button[] = 
{
    ...
    {  
       ...
       .usercode = 0xff00, 
       .nbuttons =  16, 
       .key_table = &remote_key_table_df_ir_test[0],
    },
};
...
{	
    ...
    //unsigned char keyData = ((ddata->scanData) & 0xff);
    //printk("****[ldx debug]keyData:%d\n", keyData);
...
static void remotectl_do_something(unsigned long  data)
{
    ...
    //printk("[ldx debug] ddata->state:%d ddata->period:%d ddata->count:%d\n", ddata->state, ddata->period, ddata->count);
    switch (ddata->state)
    {
        case RMC_IDLE:
        {
        ...
        case RMC_USERCODE:
        {
            ...
            if (ddata->count == 0x10){//16 bit user code
               //printk("[ldx infrared debug]u=0x%x\n",((ddata->scanData)&0xFFFF));
        ...
        case RMC_GETDATA:
        {
            ...
            //printk("[ldx debug] ddata->scanData :%d\n", ddata->scanData);           
            if (ddata->count == 0x10){
                //printk(KERN_ERR "[ldx debug]d=%x\n",(ddata->scanData&0xFFFF));
...
struct rkxx_remotectl_drvdata *g_ddata;
#if 0
static irqreturn_t remotectl_isr(int irq, void *dev_id)
{
    ...
    struct timeval  ts;
    printk("[ldx debug] remotectl_isr\n");
...
#else
static void remotectl_isr(void)
{
    struct rkxx_remotectl_drvdata *ddata =  g_ddata;
    struct timeval  ts;
    #if 0 
    //ldx own
    static current_enit_state = EINTF_TRIGGER_RISING;
    #endif
    //static current_enit_state = EINTF_TRIGGER_FALLING;
    //printk("[ldx debug] remotectl_isr\n");
    #if 0
    if (current_enit_state == EINTF_TRIGGER_FALLING)
    {
        mt_eint_set_polarity(CUST_EINT_INFRARED_RECV_NUM, (current_enit_state & EINTF_TRIGGER_FALLING) ? MT_EINT_POL_NEG : MT_EINT_POL_POS);
        current_enit_state = EINTF_TRIGGER_RISING;
    }
    else
    {
        mt_eint_set_polarity(CUST_EINT_INFRARED_RECV_NUM, (current_enit_state & EINTF_TRIGGER_FALLING) ? MT_EINT_POL_NEG : MT_EINT_POL_POS);
        current_enit_state = EINTF_TRIGGER_FALLING;
    }
    #endif
    //mt_eint_set_polarity(CUST_EINT_INFRARED_RECV_NUM, CUST_EINT_INFRARED_RECV_TYPE);
    mt_eint_unmask(CUST_EINT_INFRARED_RECV_NUM);
    //return;
    ddata->pre_time = ddata->cur_time;
    do_gettimeofday(&ts);
    ddata->cur_time = ts.tv_usec;
    #if 0 
    //ldx own
    if (current_enit_state == EINTF_TRIGGER_RISING)
    {
        if (ddata->cur_time && ddata->pre_time)
        {
            ddata->period =  ddata->cur_time - ddata->pre_time;
    ...
    tasklet_hi_schedule(&ddata->remote_tasklet); 
    if ((ddata->state==RMC_PRELOAD)||(ddata->state==RMC_SEQUENCE))
    mod_timer(&ddata->timer,jiffies + msecs_to_jiffies(230));
#ifdef CONFIG_PM
    //wake_lock_timeout(&ddata->remotectl_wake_lock, HZ);
   if ((get_suspend_state())&&(ddata->remotectl_suspend_data.cnt<50))
       ddata->remotectl_suspend_data.scanTime[ddata->remotectl_suspend_data.cnt++] = ddata->period;
...
static int __devinit remotectl_probe(struct platform_device *pdev)
{
  ...
  //error = gpio_request(pdata->gpio, "remotectl");
	...
	//	goto fail1;
	//}
	irq = CUST_EINT_INFRARED_RECV_NUM;
    #if 0
	error = request_irq(irq, remotectl_isr,	EINTF_TRIGGER_FALLING , "remotectl", ddata);
    #else
    //ldx add for test

	//mt_set_gpio_mode(GPIO_INFRARED_EINT_PIN, GPIO_INFRARED_EINT_PIN_M_EINT);
    //mt_set_gpio_dir(GPIO_INFRARED_EINT_PIN, GPIO_DIR_IN);
    //mt_set_gpio_pull_enable(GPIO_INFRARED_EINT_PIN, GPIO_PULL_DISABLE); //To disable GPIO PULL.

	//mt_eint_set_hw_debounce(CUST_EINT_INFRARED_RECV_NUM, CUST_EINT_INFRARED_RECV_DEBOUNCE_CN);
    //mt_eint_set_hw_debounce(CUST_EINT_INFRARED_RECV_NUM, 0);
	mt_eint_registration(CUST_EINT_INFRARED_RECV_NUM, CUST_EINT_INFRARED_RECV_TYPE, remotectl_isr, 0);
    //mt_eint_dis_hw_debounce(CUST_EINT_INFRARED_RECV_NUM);
	#endif
	mt_eint_unmask(CUST_EINT_INFRARED_RECV_NUM);  
